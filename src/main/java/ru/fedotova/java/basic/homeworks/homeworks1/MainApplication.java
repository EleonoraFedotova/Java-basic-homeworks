...
Метод должен посчитать их сумму, и если она больше или равна 0, то вывести в консоль сообщение “Сумма положительная”, в противном случае - “Сумма отрицательная”; */
    ...
    public static void taskAsterisk() {
        ...
        if (inNumber == 2) {
            ...
            int b = (int) (Math.random() * 20) + 1; //1-20
        ...
        if (inNumber == 3) {
            int data = (int) (Math.random() * 100) + 1; //1-100
            if (data <= 10) {
                System.out.println("Красный");
            }
            if (data > 10 && data <= 20) {
        ...
        if (inNumber == 5) {
            int initValue =(int) (Math.random() * 100) + 1; //1-100
            int delta=(int) (Math.random() * 100) + 1; //1-100
            int result1;
            System.out.println("Введите значение true или false");
